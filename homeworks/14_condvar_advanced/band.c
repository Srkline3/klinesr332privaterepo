...
// pthread_cond_t *drummer, *guitarist, *singer;
// pthread_cond_t conds[] = {drummer, guitarist, singer};

pthread_cond_t waiting_to_play;
pthread_cond_t waiting_for_stage;
bool stage[] = {false, false, false};
int musicians = 0;
...
void *friend(void *kind_ptr)
{
  ...
  while (stage[kind])
  {
    pthread_cond_wait(&waiting_for_stage, &lock);
  }
  stage[kind] = true;
  musicians++;

  while (!stage[DRUM] || !stage[SING] || !stage[GUIT])
  {
    pthread_cond_wait(&waiting_to_play, &lock);
  }

  pthread_cond_broadcast(&waiting_to_play);
  ...
  if (musicians == 0)
  {
    pthread_cond_broadcast(&waiting_for_stage);
  }
