...
bool validThreads[5] = {false, false, false, false, false};
...
void create_new_parameterized_thread(void (*fun_ptr)(void*), void* parameter) {
        printf("New parameterized thread\n");
    if(currentThreadCount > 4){
    ...
    int index = 0;
    printf("New thread loop time: %d\n", currentThreadCount);
    while(!threadAdded){
        if(!validThreads[index]){
            ...
            void(*fun_ptr)() = &thread_finisher; 

            makecontext(&threads[index], fun_ptr, 2, cast_ptr, parameter); 
            
            validThreads[index] = true;
            threadAdded = true;
        }else{
            index++;
        }
    }
    printf("New thread loop done\n");
    currentThreadCount++;   
}
...
void yield() {
    printf("yieldo=%d\n", currentThreadIndex);
    swapcontext(&threads[currentThreadIndex], &scheduler);
}
...
void finish_thread() {
    currentThreadCount--;
    validThreads[currentThreadIndex] = false;
    printf("%d \t %d\n", currentThreadCount, currentThreadIndex);
    swapcontext(&threads[currentThreadIndex], &scheduler);
}
